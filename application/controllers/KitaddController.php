<?php
class KitaddController extends Zend_Controller_Action{
	...
	public function buycontractAction()
	{
		...
		//	$contract_price = !empty($InvoiceDetail->total_price)?$InvoiceDetail->total_price:'';
			...
			}else if($contract_id == 4){
				$contract_type = "Silver";
			}else if($contract_id == 2 || $contract_id == 7){
				$contract_type = "ROI";
			}else{
			...
			$sha_hashrate_qty=$common_obj->cleanQueryParameter(($_POST['sha_hashrate_qty']));
			$db->commit();
			$arr=array('success'=>'success','failure'=>'','contract_type'=>$contract_type,'contract_id'=>$contract_id,'value'=>$value,'amount'=>$hid_btc_value,'sha_hashrate_qty'=>$sha_hashrate_qty,'type'=>$type,'k_type'=>$k_type);
