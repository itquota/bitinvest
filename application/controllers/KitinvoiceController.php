<?php
class KitinvoiceController extends Zend_Controller_Action{
	...
	public function indexAction()
	{
		...
		try{
	 	...
	 	$MaxRowsCount=100;
			...
			->from(array('kit_invoices'=>'kit_invoices'),array('count(invoice_id) as total_count'))
			->where("kit_invoices.contract_rate <>?", '0');
			...
			*/
				...
				$result = $kitinvoicesObj->select()
				->setIntegrityCheck(false)
				->from(array('kit_invoices'=>'kit_invoices'),array('kit_invoices.username','kit_invoices.middleAddr', 'kit_invoices.kits_qty', 'kit_invoices.creared_on as created_on', 'kit_invoices.invoice_id', 'kit_invoices.contract_rate','kit_invoices.amtPaid', 'kit_invoices.amtPaidInMcap',  'kit_invoices.transactionid', 'kit_invoices.comment', 'kit_invoices.invoice_status'
		/*,'(select payment_mode from kits_payments where kits_payments.invoice_id = kit_invoices.invoice_id group by invoice_id) as payment_mode'*/
				...
				->joinInner(array('k1'=>'kits'),'(k1.invoice_id=kit_invoices.invoice_id)',array('k1.contract_id', 'k1.comment as comments'))
