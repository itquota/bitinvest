<?php
class FinledgercronController extends Zend_Controller_Action{
	...
	/*		mysql_query("CREATE TABLE `final_balance_$date` LIKE final_balance");
			...
			$checkRefundDate = !empty($_GET['checkRefundDate'])?$_GET['checkRefundDate']:REFUND_DATE;
			...
			while($count < ($userCount->userCount - 1)){
				...
				foreach($Users as $username){
					...
					->setIntegrityCheck(false)
					->from(array('d' =>'daily_ledger'),array('round(SUM(daily_earning_amt),8) as daily_earning_amt','round(SUM(ref_amt),8) as ref_amt','round(SUM(binary_amt),8) as binary_amt','round(SUM(daily_earning_amt_new),8) as daily_earning_amt_new','total_amt'))
					...
					$count++;
					$UpdateFinalLedger = $common_obj->UpdateFinalLedger('', $username);
				}
