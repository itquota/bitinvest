...
class GenerateethinvoiceController extends Zend_Controller_Action
{
    ...
    public function indexAction(){
        try {
            ...
            $invoicesObj=new Gbc_Model_DbTable_Gb2Invoices();
			...
			if($ContractID > 13){
            	...
            	$pdf->Multicell( 82, 6, "1. ETH MINING CONTRACT \n   ( $contractQty MH/s)" , 1, 'L', false );
			}
            $pdf->SetXY($x + 82, $y+59);

            $pdf->Cell( 36, 12, '1', 1, 0, 'C', false );
			if($ContractID > 13){
				$pdf->Cell( 36, 12, "$contractRate MCAP", 1, 0, 'C', false );
				$pdf->Cell( 36, 12, "$contractRate MCAP", 1, 0, 'C', false );
			}else{
				$pdf->Cell( 36, 12, "$contractRate MCAP", 1, 0, 'C', false );
				$pdf->Cell( 36, 12, "$totalPaid MCAP", 1, 0, 'C', false );
			}
		...
		/*	if($ContractID > 13){
			...
			*/
				$pdf->Cell( 36, 12, 'Subtotal', 1, 0, 'C', false );
				$pdf->Cell( 36, 12, 'MCAP  '.$totalPaid, 1, 0, 'C', false );
			//}
       ...
       /*     if($ContractID > 13){
			...
			*/
				$pdf->Cell( 36, 12, 'TOTAL', 0, 0, 'C', true );
				$pdf->Cell( 36, 12, 'MCAP  '.$totalPaid, 0, 0, 'C', true );
		//
            $pdf->Ln( 12 );

            $pdf->SetTextColor( $tableHeaderTopProductTextColour[0], $tableHeaderTopProductTextColour[1], $tableHeaderTopProductTextColour[2] );
            $pdf->SetFillColor( $tableHeaderTopProductFillColour[0], $tableHeaderTopProductFillColour[1], $tableHeaderTopProductFillColour[2] );
            $pdf->Cell( 82, 12, " **Amount In MCAP Only", 0, 0, 'L', true );
